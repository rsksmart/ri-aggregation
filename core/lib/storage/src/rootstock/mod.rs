...
use zksync_types::{
    ...
    rootstock::{InsertedOperationResponse, RSKOperation},
    BlockNumber, H256, U256,
};
// Local imports
use self::records::{RSKOperationData, RSKParams, RSKStats, RSKTxHash, StorageRSKOperation};
...
impl<'a, 'c> RootstockSchema<'a, 'c> {
    ...
    pub async fn load_unconfirmed_operations(&mut self) -> QueryResult<VecDeque<RSKOperation>> {
        let start = Instant::now();
        // Load the operations with the associated Rootstock transactions
        // from the database.
        // Here we obtain a sequence of one-to-one mappings (RSK tx) -> (operation ID).
        ...
        // be RSK transactions without an operation (e.g. `completeWithdrawals` call), but for
        // every operation always there is an RSK transaction.

        let mut transaction = self.0.start_transaction().await?;

        let rsk_ops: Vec<RSKOperationData> = sqlx::query_as!(
            RSKOperationData,
            r#"
                SELECT rsk_operations.*,
                    aggregate_operations.id as "agg_op_id?",
                    aggregate_operations.arguments as "arguments?"
                FROM rsk_operations
                LEFT JOIN rsk_aggregated_ops_binding
                    ON rsk_aggregated_ops_binding.rsk_op_id = rsk_operations.id
                LEFT JOIN aggregate_operations
                    ON aggregate_operations.id = rsk_aggregated_ops_binding.op_id
                WHERE rsk_operations.confirmed = false
                ORDER BY rsk_operations.id ASC
            "#,
        )
        ...
        let mut ops: VecDeque<RSKOperation> = VecDeque::with_capacity(rsk_ops.len());

        // Transform the `StoredOperation` to `Operation` and `StoredRSKOperation` to `RSKOperation`.
        for rsk_op in rsk_ops {
            // Load the stored txs hashes ordered by their ID,
            // so the latest added hash will be the last one in the list.
            let rsk_tx_hashes: Vec<RSKTxHash> = sqlx::query_as!(
                RSKTxHash,
                "SELECT * FROM rsk_tx_hashes
                WHERE rsk_op_id = $1
                ORDER BY id ASC",
                rsk_op.id
            )
            ...
            .await?;
            assert!(
                !rsk_tx_hashes.is_empty(),
                "No hashes stored for the Rootstock operation"
            );

            // If there is an operation, convert it to the `AggregatedOperation` type.
            let op = rsk_op.agg_op_id.map(|id| {
                let op: AggregatedOperation =
                    serde_json::from_value(rsk_op.arguments.clone().unwrap())
                        .expect("Incorrect serialized aggregated operation in storage");
                (id, op)
            });

            // Convert the fields into expected format.
            let op_type = AggregatedActionType::from_str(rsk_op.op_type.as_ref())
                .expect("Stored operation type must have a valid value");
            let last_used_gas_price =
                U256::from_str(&rsk_op.last_used_gas_price.to_string()).unwrap();
            let used_tx_hashes = rsk_tx_hashes
                .iter()
                .map(|entry| H256::from_slice(&entry.tx_hash))
                .collect();
            let final_hash = rsk_op.final_hash.map(|hash| H256::from_slice(&hash));

            let rsk_op = RSKOperation {
                id: rsk_op.id,
                op_type,
                op,
                nonce: rsk_op.nonce.into(),
                last_deadline_block: rsk_op.last_deadline_block as u64,
                last_used_gas_price,
                used_tx_hashes,
                encoded_tx_data: rsk_op.raw_tx,
                confirmed: rsk_op.confirmed,
                final_hash,
            };

            ops.push_back(rsk_op);
        }
        ...
        sqlx::query!(
            "WITH aggregate_ops AS (
                SELECT aggregate_operations.id FROM aggregate_operations
                   WHERE confirmed = $1 and action_type != $2 and aggregate_operations.id != ANY(SELECT id from rsk_aggregated_ops_binding)
    ...
    /// and are in `rsk_unprocessed_aggregated_ops`.
    pub async fn load_unprocessed_operations(
        ...
        let raw_ops = sqlx::query_as!(
            ...
            WHERE EXISTS (SELECT * FROM rsk_unprocessed_aggregated_ops WHERE op_id = aggregate_operations.id)
            ORDER BY id ASC
            "#,
        )
    ...
    /// Removes the given IDs from `rsk_unprocessed_aggregated_ops`.
    ...
    pub async fn remove_unprocessed_operations(
        ...
        sqlx::query!(
            "DELETE FROM rsk_unprocessed_aggregated_ops WHERE op_id = ANY($1)",
            &operations_id
        )
        .execute(self.0.conn())
        .await?;
    ...
    /// Returns the `RSKOperation` object containing the assigned nonce and operation ID.
    pub async fn save_new_rsk_tx(
        &mut self,
        ...
        let rsk_op_id = sqlx::query!(
            "
                INSERT INTO rsk_operations (op_type, nonce, last_deadline_block, last_used_gas_price, raw_tx)
                VALUES ($1, $2, $3, $4, $5)
                RETURNING id
            ",
        ...
        if let Some((op_id, op)) = operation {
            sqlx::query!(
                "INSERT INTO rsk_aggregated_ops_binding (op_id, rsk_op_id) VALUES ($1, $2)",
                op_id,
                rsk_op_id
            )
        ...
        metrics::histogram!("sql.rootstock.save_new_rsk_tx", start.elapsed());
        Ok(response)
    }
    ...
    pub async fn is_aggregated_op_confirmed(&mut self, id: i64) -> QueryResult<bool> {
        ...
        let confirmed = sqlx::query_as!(
            StorageRSKOperation,
            "SELECT * FROM rsk_operations WHERE id <= $1 ORDER BY ID DESC LIMIT 1",
            id
        )
    ...
    async fn get_rsk_op_id(&mut self, hash: &H256) -> QueryResult<i64> {
        let start = Instant::now();
        let hash_entry = sqlx::query_as!(
            RSKTxHash,
            "SELECT * FROM rsk_tx_hashes WHERE tx_hash = $1",
            hash.as_bytes()
        )
        .fetch_one(self.0.conn())
        .await?;

        metrics::histogram!("sql.rootstock.get_rsk_op_id", start.elapsed());
        Ok(hash_entry.rsk_op_id)
    }

    /// Adds a tx hash entry associated with some Rootstock operation to the database.
    pub async fn add_hash_entry(&mut self, rsk_op_id: i64, hash: &H256) -> QueryResult<()> {
        let start = Instant::now();
        // Insert the new hash entry.
        sqlx::query!(
            "INSERT INTO rsk_tx_hashes (rsk_op_id, tx_hash) VALUES ($1, $2)",
            rsk_op_id,
            hash.as_bytes()
        )
        .execute(self.0.conn())
    ...
    pub async fn update_rsk_tx(
        ...
        metrics::histogram!("sql.rootstock.update_rsk_tx", start.elapsed());
        Ok(())
    }
    ...
    /// stats values. Currently the script `db-insert-rsk-data.sh` is responsible for that
    ...
    async fn report_created_operation(
        ...
        let mut current_stats = RootstockSchema(&mut transaction).load_rsk_params().await?;
    ...
    ///
    /// This method expects the database to be initially prepared with inserting the actual
    /// gas limit value. The command responsible for that is `zk db insert rsk-data`.
    ...
    pub async fn load_gas_price_limit(&mut self) -> QueryResult<U256> {
        let start = Instant::now();
        let params = self.load_rsk_params().await?;
    ...
    pub async fn load_stats(&mut self) -> QueryResult<RSKStats> {
        let start = Instant::now();
        let params = self.load_rsk_params().await?;
    ...
    async fn load_rsk_params(&mut self) -> QueryResult<RSKParams> {
        let start = Instant::now();
        let params = sqlx::query_as!(RSKParams, "SELECT * FROM rsk_parameters WHERE id = true",)
            .fetch_one(self.0.conn())
            .await?;
        metrics::histogram!("sql.rootstock.load_rsk_params", start.elapsed());
        Ok(params)
    }

    /// Marks the stored Rootstock transaction as confirmed (and thus the associated `Operation`
    /// is marked as confirmed as well).
    pub async fn confirm_rsk_tx(&mut self, hash: &H256) -> QueryResult<()> {
        let start = Instant::now();
        let mut transaction = self.0.start_transaction().await?;

        let rsk_op_id = RootstockSchema(&mut transaction)
            .get_rsk_op_id(hash)
            .await?;
        ...
        let aggregated_op = sqlx::query_as!(
            ...
            "SELECT * FROM aggregate_operations
                WHERE id = (SELECT op_id FROM rsk_aggregated_ops_binding WHERE rsk_op_id = $1)",
            rsk_op_id,
        )
        ...
        let created_at_time = RootstockSchema(&mut transaction)
            .get_rsk_operation_creation_time(rsk_op_id)
            .await?;
        if let Some(time) = created_at_time {
            ...
            metrics::histogram!("rsk_operation_confirmation", duration);
        }

        transaction.commit().await?;

        metrics::histogram!("sql.rootstock.confirm_rsk_tx", start.elapsed());
        Ok(())
    }
    ...
    ///
    /// This method expects the database to be initially prepared with inserting the actual
    /// nonce value. Currently the script `db-insert-rsk-data.sh` is responsible for that
    /// and it's invoked within `db-reset` subcommand.
    #[doc = "hidden"]
    pub async fn get_next_nonce(&mut self) -> QueryResult<i64> {
        ...
        let old_nonce: RSKParams = RootstockSchema(&mut transaction).load_rsk_params().await?;
    ...
    /// Method that internally initializes the `rsk_parameters` table.
    /// Since in db tests the database is empty, we must provide a possibility
    /// to initialize required db fields.
    #[doc = "hidden"]
    pub async fn initialize_rsk_data(&mut self) -> QueryResult<()> {
        let start = Instant::now();
        #[derive(Debug)]
        pub struct NewRSKParams {
            pub nonce: i64,
        ...
        let old_params: Option<RSKParams> =
            sqlx::query_as!(RSKParams, "SELECT * FROM rsk_parameters WHERE id = true",)
                .fetch_optional(self.0.conn())
                .await?;

        if old_params.is_none() {
            let params = NewRSKParams {
                nonce: 0,
            ...
            sqlx::query!(
                "INSERT INTO rsk_parameters (nonce, gas_price_limit, last_committed_block, last_verified_block, last_executed_block)
                VALUES ($1, $2, $3, $4, $5)",
        ...
        metrics::histogram!("sql.rootstock.initialize_rsk_data", start.elapsed());
        Ok(())
    }

    async fn get_rsk_operation_creation_time(
        &mut self,
        op_id: i64,
    ) -> QueryResult<Option<DateTime<Utc>>> {
        let start = Instant::now();
        let created_at = sqlx::query!(
            "SELECT created_at FROM rsk_operations WHERE id = $1",
            op_id as i64
        )
        .fetch_one(self.0.conn())
        .await?
        .created_at;
        metrics::histogram!(
            "sql.rootstock.get_rsk_operation_creation_time",
            start.elapsed()
        );
        Ok(created_at)
    }
    ...
    pub async fn aggregated_op_final_hash(
        ...
        let rsk_operation = sqlx::query_as!(
            StorageRSKOperation,
            "SELECT rsk_operations.* FROM aggregate_operations
                LEFT JOIN rsk_aggregated_ops_binding ON rsk_aggregated_ops_binding.op_id = aggregate_operations.id
                LEFT JOIN rsk_operations ON rsk_aggregated_ops_binding.rsk_op_id = rsk_operations.id
            WHERE
                ($1 BETWEEN from_block AND to_block) AND action_type = $2 AND rsk_operations.confirmed = true 
            LIMIT 1",
        ...
        let final_hash = rsk_operation
            .map(|rsk_operation| rsk_operation.final_hash.map(|hash| H256::from_slice(&hash)))
            .flatten();

        Ok(final_hash)
    }

    // Updates rsk_parameters with given nonce and last block.
    // It updates last_verified_block only if it is greater than given last block.
    pub async fn update_rsk_parameters(&mut self, last_block: BlockNumber) -> QueryResult<()> {
        let start = Instant::now();
        let mut transaction = self.0.start_transaction().await?;
        sqlx::query!(
            "UPDATE rsk_parameters SET last_committed_block = $1 WHERE id = true",
            *last_block as i64
        )
        .execute(transaction.conn())
        .await?;
        ...
        sqlx::query!(
            "UPDATE rsk_parameters SET last_verified_block = $1 WHERE id = true AND last_verified_block > $1",
            *last_block as i64
        )
        .execute(transaction.conn())
        .await?;
        ...
        metrics::histogram!("sql.rootstock.update_rsk_parameters", start.elapsed());
        Ok(())
    }
}
