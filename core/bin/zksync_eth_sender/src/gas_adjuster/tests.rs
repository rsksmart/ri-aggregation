...
async fn eth_and_db_clients() -> (RootstockGateway, MockDatabase) {
    ...
    (eth_sender.rootstock, eth_sender.db)
}
...
async fn initial_price() {
    let (mut rootstock, db) = eth_and_db_clients().await;
    ...
    // Vector of rootstock client prices.
    let test_vector = vec![
        0,
        13,
    ...
    for eth_client_price in test_vector {
        ...
        let scaled_gas = gas_adjuster.get_gas_price(&rootstock, None).await.unwrap();
...
async fn lower_gas_limit() {
    ...
    // Test vector of pairs (rootstock client price, price of the last tx, expected price).
    let test_vector = vec![
    ...
    for (eth_client_price, previous_price, expected_price) in test_vector {
        ...
        let scaled_gas = gas_adjuster
            .get_gas_price(&rootstock, Some(previous_price.into()))
            .await
            .unwrap();
...
async fn initial_upper_gas_limit() {
    ...
    // Check that gas price is clamped even if both the rootstock client price
    ...
    let scaled_gas = gas_adjuster
        .get_gas_price(&rootstock, Some(previous_price))
        .await
        .unwrap();
    assert_eq!(scaled_gas, PRICE_LIMIT.into());
}
...
async fn average_gas_price_stored_correctly() {
    ...
    gas_adjuster.keep_updated(&rootstock, &db).await;
...
/// rootstock client suggesting the price far beyond the current limit
...
async fn gas_price_limit_scaling() {
    ...
    for _ in 0..N_SAMPLES / 2 {
        let suggested_price = gas_adjuster
            .get_gas_price(&rootstock, Some(expected_price.into()))
            .await
            .unwrap();
    ...
    for _ in 0..PRICE_UPDATES {
        ...
        // Also, scale up the price reported by the rootstock.
        rootstock
            .get_mut_mock()
            .unwrap()
...
async fn gas_price_limit_preservation() {
    ...
    for _ in 0..PRICE_UPDATES {
        ...
        for _ in 0..N_SAMPLES {
            ...
            // than the limit).
            let suggested_price = gas_adjuster.get_gas_price(&rootstock, None).await.unwrap();
