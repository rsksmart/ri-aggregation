...
impl ApiV01 {
    ...
    pub async fn tokens_acceptable_for_fees(self_: web::Data<Self>) -> ActixResult<HttpResponse> {
        ...
        let unconditionally_valid_addresses = ticker_config.unconditionally_valid_tokens;
        ...
        let tokens_to_filter: HashMap<TokenId, Token> = tokens
            .into_iter()
            .filter(|(_, token)| {
                let is_unconditionally_valid =
                    unconditionally_valid_addresses.contains(&token.address);
                if is_unconditionally_valid {
                    unconditionally_valid_tokens.push(token.clone());
                }

                !is_unconditionally_valid
            })
