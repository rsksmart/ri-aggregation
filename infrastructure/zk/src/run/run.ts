...
export async function deployERC20(
  ...
  if (command == "dev") {
    ...
    const localhostTokensFile = fs.readFileSync(
      "./etc/tokens/localhost.json",
      "utf-8"
    );
    const tokens: Token[] =
      localhostTokensFile && JSON.parse(localhostTokensFile);
    const deployedUnconditionallyValidAddresses =
      (tokens &&
        tokens
          .filter((token) => ["RDOC", "RIF"].includes(token.symbol))
          .map(({ address }) => address)) ||
      [];

    const feeTickerConfigFile = fs.readFileSync(
      "./etc/env/dev/fee_ticker.toml",
      "utf-8"
    );
    const feeTicker =
      feeTickerConfigFile &&
      (toml.parse(feeTickerConfigFile) as PartialFeeTickerConfig);
    let unconditionallyValidTokens =
      feeTicker && feeTicker.fee_ticker.unconditionally_valid_tokens;
...
export async function governanceAddERC20(
  command: "dev" | "new",
  address?: string
) {
  if (command == "dev") {
    await utils.spawn(
      `yarn --silent --cwd contracts governance-add-erc20 add-multi-current-network localhost`
    );
  } else if (command == "new") {
    await utils.spawn(
      `yarn --cwd contracts governance-add-erc20 add ${address}`
    );
  }
}

export async function serverAddERC20(
  address: string,
  symbol: string,
  decimals: string
) {
...
export async function serverAddERC20AndMint(
  name: string,
  symbol: string,
  decimals: string,
  net: string
...
export async function deployTestkit(
  genesisRoot: string,
  prodContracts: boolean = false
) {
  const option = prodContracts ? "--prodContracts" : "";
  await utils.spawn(
    `yarn contracts deploy-testkit --genesisRoot ${genesisRoot} ${option}`
  );
}
...
export async function testUpgrade(contract: string, gatekeeper: string) {
  await utils.spawn(
    `yarn contracts ts-node scripts/test-upgrade-franklin.ts ${contract} ${gatekeeper}`
  );
}
...
export async function revertReason(txHash: string, web3url?: string) {
  await utils.spawn(
    `yarn contracts ts-node scripts/revert-reason.ts ${txHash} ${web3url || ""}`
  );
}
...
export async function exitProof(...args: string[]) {
  await utils.spawn(
    `cargo run --example generate_exit_proof --release -- ${args.join(" ")}`
  );
}
...
export async function testAccounts() {
  const testConfigPath = path.join(
    ...
    `etc/test_config/constant`
  );
  const ethTestConfig = JSON.parse(
    fs.readFileSync(`${testConfigPath}/eth.json`, { encoding: "utf-8" })
  );
  const walletKeys = [];

  let ethWallet = new Wallet(
    Buffer.from(ethTestConfig.account_with_rbtc_cow_privK, "hex")
  );
  walletKeys.push({
  ...
  ethWallet = new Wallet(
    Buffer.from(ethTestConfig.account_with_rbtc_cow1_privK, "hex")
  );
  walletKeys.push({
    address: ethWallet.address,
    privateKey: ethWallet.privateKey,
  ...
  ethWallet = new Wallet(
    Buffer.from(ethTestConfig.account_with_rbtc_cow2_privK, "hex")
  );
  walletKeys.push({
    address: ethWallet.address,
    privateKey: ethWallet.privateKey,
  ...
  ethWallet = new Wallet(
    Buffer.from(ethTestConfig.account_with_rbtc_cow3_privK, "hex")
  );
  walletKeys.push({
    address: ethWallet.address,
    privateKey: ethWallet.privateKey,
  ...
  ethWallet = new Wallet(
    Buffer.from(ethTestConfig.account_with_rbtc_cow4_privK, "hex")
  );
  walletKeys.push({
    address: ethWallet.address,
    privateKey: ethWallet.privateKey,
  ...
  ethWallet = new Wallet(
    Buffer.from(ethTestConfig.account_with_rbtc_cow5_privK, "hex")
  );
  walletKeys.push({
    address: ethWallet.address,
    privateKey: ethWallet.privateKey,
  ...
  //});
  ethWallet = new Wallet(
    Buffer.from(ethTestConfig.account_with_rbtc_cow7_privK, "hex")
  );
  walletKeys.push({
    address: ethWallet.address,
    privateKey: ethWallet.privateKey,
  ...
  ethWallet = new Wallet(
    Buffer.from(ethTestConfig.account_with_rbtc_cow8_privK, "hex")
  );
  walletKeys.push({
    address: ethWallet.address,
    privateKey: ethWallet.privateKey,
  ...
  ethWallet = new Wallet(
    Buffer.from(ethTestConfig.account_with_rbtc_cow9_privK, "hex")
  );
  walletKeys.push({
    address: ethWallet.address,
    privateKey: ethWallet.privateKey,
...
command
  ...
  .description("print rootstock test accounts")
  .action(testAccounts);
command
  .command("explorer")
...
command
  .command("test-upgrade <main_contract> <gatekeeper_contract>")
  .action(testUpgrade);
command
...
command
  ...
  .action(
    async (name: string, symbol: string, decimals: string, net: string) => {
      await utils.confirmAction();
