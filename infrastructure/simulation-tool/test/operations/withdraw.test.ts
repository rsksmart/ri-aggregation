...
import {
  ...
  TransactionReceipt,
} from "@rsksmart/rif-rollup-js-sdk/build/types";
...
describe("executeWithdraw", () => {
  it("should execute deposit", async () => {
    const l2sender = sinon.createStubInstance(RollupWallet);
    l2sender.withdrawFromSyncToRootstock.callsFake(() =>
      Promise.resolve(sinon.createStubInstance(Transaction))
    );
    const withdrawal = prepareWithdrawal(l2sender);
    const expectedParameters = { ...withdrawal };
    await executeWithdrawal(withdrawal);

    expect(l2sender.withdrawFromSyncToRootstock).to.have.been.calledOnceWith(
      expectedParameters
    );
  });
...
describe("resolveTransaction", () => {
  it("should return deposit result", async () => {
    ...
    withdrawOp.awaitVerifyReceipt.callsFake(() =>
      Promise.resolve(expectedL2VerifyReceipt)
    );
...
describe("executeWithdrawals", () => {
  it("should execute all withdrawals", async () => {
    ...
    const users = [...Array(5)].map(() => {
      ...
      l2Wallet.withdrawFromSyncToRootstock.resolves(
        sinon.createStubInstance(Transaction)
      );
      return l2Wallet;
    });
