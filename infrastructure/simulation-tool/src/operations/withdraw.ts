...
type PreparedWithdrawal = Parameters<
  RollupWallet["withdrawFromSyncToRootstock"]
>[number] & {
  amount: BigNumber;
  from: RollupWallet;
};
...
const executeWithdrawal = async (
  ...
  console.log(
    `Withdrawing ${
      preparedWithdrawal.amount
    } RBTC from ${preparedWithdrawal.from.address()}`
  ); // commented out to reduce noise (TODO: create more sophisticated logging)

  return preparedWithdrawal.from.withdrawFromSyncToRootstock(
    preparedWithdrawal
  );
};
    ...
    console.log(
      `Verification of block #${verification.block.blockNumber} ${
        verification.success
          ? "successful"
          : "failed with: " + verification.failReason
      }.`
    );
